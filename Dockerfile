...
ENV JARVIS_TIMEZONE=Africa/Johannesburg

WORKDIR /app

# 1. Install system dependencies: audio libraries, build tools, Node.js & npm (for MCP servers), tzdata
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    ca-certificates \
    gnupg \
    && ln -snf /usr/share/zoneinfo/Africa/Johannesburg /etc/localtime && echo "Africa/Johannesburg" > /etc/timezone \
